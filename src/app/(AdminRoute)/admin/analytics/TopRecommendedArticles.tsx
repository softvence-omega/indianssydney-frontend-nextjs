...
// import { useGetTopArticlesQuery } from "@/store/features/admin/analytics.api";
...
//   const { data, isLoading, isError } = useGetTopArticlesQuery({});
//   console.log("top recommended data", data);
...
const TopRecommendedArticles: React.FC = () => {
  ...
  // ✅ Extract approved articles and sort them by contentviews (descending)
  const approvedArticles = data
    ?.filter((article: Article) => article.status === "APPROVE")
    ?.sort((a: Article, b: Article) => b.contentviews - a.contentviews)
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border p-6">
      ...
      {approvedArticles.length === 0 ? (
        <p className="text-sm text-gray-500">No approved articles yet.</p>
      ) : (
        <div className="space-y-3">
          {approvedArticles.map((article: Article, index: number) => (
            <div
              key={article.id}
              className="flex flex-col sm:flex-row sm:items-center gap-4 sm:justify-between py-2 border-b border-gray-100 last:border-b-0"
            >
