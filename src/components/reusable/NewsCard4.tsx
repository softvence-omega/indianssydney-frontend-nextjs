  ...
  return (
    <Link href={`/details/article/${id}`} >
      <div className={`relative w-full overflow-hidden ${imgHeight}`}>
        ...
        {/* Overlay with blur */}
        <div
          className={`absolute inset-0 bg-gradient-to-t from-black to-transparent ${type === "transparent" ? "opacity-30" : "opacity-90"
            }`}
        />
        ...
        <div className="absolute bottom-0 left-0 right-0 z-1">
          <div
            className={`p-4 text-white ${type === "transparent"
          ...
          >
            {tag && (
              <span
                className={`text-xs font-bold uppercase border px-2 py-1 mb-3 inline-block ${type === "transparent"
            ...
            <div
              className={`text-xs lg:text-sm font-medium ${type === "transparent" ? "text-white" : "text-accent-orange"
                }`}
            >
